...
- the approved concise global authority capsule in `~/.codex/AGENTS.md` includes the score/verification discipline, and the approved `UserPromptSubmit` runtime hook binds the score layer every task turn without replacing the explicit verify chain
...
- generated `hooks.json` is approved only when it exactly matches the scorecard `UserPromptSubmit` hook. It must use `user_prompt_throttle_seconds = 0`; if authority disables `runtime_hook` or clears its events, the generated hook file should disappear instead of leaving stale reminders behind.
